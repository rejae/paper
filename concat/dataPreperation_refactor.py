...
# train_names=[ "train_set"+str(i)+".txt" for i in range(0,18,2)]
# test_names =[ "test_set"+str(i)+".txt" for i in range(0,18,2)]
...
for train_name,test_name in zip(train_names,test_names):
    ...
    for i in range(6):
        ...
        modeldir = os.path.join(outdir,  ''.join(train_name)+'current'+str(i)) #模型名字为train_set+'i'

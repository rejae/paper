...
labels_dir =  "data/cateAndQuest.txt"

words_dir =  "data/vocabs.txt"  
...
tf.flags.DEFINE_integer("max_seq_len", 30, "max length of sequence")
...
tf.flags.DEFINE_integer("num_train_epochs", 30, "Number of train data set training epochs (default: 200)")   # 100
...
def preprocess(train,test):
    ...
    train_x, train_x1,train_y, train_label_num,train_asr_data,train_trs_data = data_helper.load_data(train,test=False)
    test_x, test_x1,test_y, test_label_num,test_asr_data,test_trs_data  = data_helper.load_data(test,test=True)
...
#                     feed_dict)
                _, step, loss,accuracy,accuracy_trs = sess.run(
                    [train_op, global_step, cnn.loss,cnn.accuracy_asr,cnn.accuracy_trs],
                    feed_dict)
                time_str = datetime.datetime.now().isoformat()
                log.info("{}: step {}, loss {:g}, acc {:g}, acc_trs {:g}".format(time_str, step, loss, accuracy, accuracy_trs)) #########
            ...
            def testDataEvaluate(test_x,test_x1, test_y, writer=None):
                ...
                step, loss_asr, accuracy_asr, predictions_asr,scores_asr,scores_trs = sess.run(
                    [global_step, cnn.loss, cnn.accuracy_asr, cnn.predictions_asr,cnn.scores_asr,cnn.scores_trs],
                    feed_dict)
                time_str = datetime.datetime.now().isoformat()
                log.info("----------------------------------------------------")
...
#                 id2label = ['AddToPlaylist','BookRestaurant','GetWeather','PlayMusic','RateBook','SearchCreativeWork','SearchScreeningEvent']
...
#     a,b,c = [1],[0],[1]
    for i,j,k in zip(a,b,c):
        e1,e2,e3=i,j,k
#         train_names = ['nbest_snips_train.csv'] 
#         test_names = ['nbest_snips_test.csv']  
        train_names = ['nbest_snips_train.csv','nbest_snips_train.csv','nbest_snips_train.csv','nbest_snips_train.csv','nbest_snips_train.csv'] 
        test_names = ['nbest_snips_test.csv','nbest_snips_test.csv','nbest_snips_test.csv','nbest_snips_test.csv','nbest_snips_test.csv']  
        count = 0

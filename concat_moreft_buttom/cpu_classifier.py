...
flags.DEFINE_string(
    "am_lm", "lm",
    "Use am or lm or both to weighting.")
...
flags.DEFINE_integer("max_headers_size", 20, "Total batch size.")
flags.DEFINE_integer("max_extra_features_size", 20, "Total batch size.")
...
class InputExample(object):
    ...
    def __init__(self, guid, text_a, text_b=None, label=None, header_ids=None, extra_features=None):
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_id, header_ids, extra_features):
...
class DataProcessor(object):
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            header_ids=tokenization.convert_to_unicode(line[3])
            extra_features=tokenization.convert_to_unicode(line[4])
            examples.append(InputExample(guid=guid, text_a=text_a, text_b=None, label=label, header_ids=header_ids, extra_features=extra_features))
        return examples
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
                           tokenizer,max_headers_size, max_extra_features_size):
    ...
    extra_features=[]
    extra_features_temp=[float(feature) for feature in example.more_features.strip().split()]
    if FLAGS.am_lm == 'am':
        for i in range(0,len(extra_features_temp),2):
            extra_features.append(extra_features_temp[i])
    if FLAGS.am_lm == 'lm':
        for i in range(1,len(extra_features_temp),2):
            extra_features.append(extra_features_temp[i])
    if FLAGS.am_lm == 'am_lm':
        for i in range(0,len(extra_features_temp),2):
            extra_features.append((extra_features_temp[i]+extra_features_temp[i+1])/2)    
    
    header_ids=[int(idx)+1 for idx in example.header_ids.strip().split()]
    while len(header_ids) < max_headers_size:
        header_ids.append(0)
        extra_features.append(0)
    

    assert len(extra_features) == len(header_ids)
    ...
    if ex_index < 5:
        ...
        tf.logging.info("header_ids: %s" % " ".join([str(x) for x in header_ids]))
...
def file_based_convert_examples_to_features(
        examples, label_list, max_seq_length, tokenizer, output_file, max_headers_size, max_extra_features_size):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        feature = convert_single_example(ex_index, example, label_list,
                                         max_seq_length, tokenizer, max_headers_size, max_extra_features_size)
        ...
        features["header_ids"] = create_int_feature(feature.header_ids)
...
def file_based_input_fn_builder(input_file, seq_length, is_training,
                                drop_remainder,max_headers_size, max_extra_features_size):
    ...
    name_to_features = {
        ...
        "header_ids": tf.FixedLenFeature([max_headers_size], tf.int64),
        "extra_features": tf.FixedLenFeature([max_extra_features_size],tf.float32),
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
                 labels, num_labels, use_one_hot_embeddings,header_ids,extra_features):
    ...
    # instead.
    sequence_output = model.get_all_encoder_layers()[-6]  #(None,len,768)
    #first_token_tensor = tf.squeeze(sequence_output[:, 0:1, :], axis=1)
    #first_token_tensor=tf.concat([first_token_tensor,extra_features],axis=-1)
    #print(header_ids.shape)
    header_ids_tensor=tf.batch_gather(sequence_output,header_ids) #(None,20,768)
    #print(header_ids_tensor.shape)
    extra_features_tensor=tf.expand_dims(extra_features,axis=-1) #(None,20,1)
    header_ids_tensor_weighting=tf.multiply(header_ids_tensor,extra_features_tensor) #(None,20,768)
    token_tensor=tf.reduce_sum(header_ids_tensor_weighting,axis=1) #(None,768)
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        (total_loss, per_example_loss, logits, probabilities) = create_model(
            ...
            num_labels, use_one_hot_embeddings,header_ids,extra_features)
...
def main(_):
    ...
    if FLAGS.do_train:
        ...
        file_based_convert_examples_to_features(
            train_examples, label_list, FLAGS.max_seq_length, tokenizer, train_file, FLAGS.max_headers_size, FLAGS.max_extra_features_size)
        ...
        train_input_fn = file_based_input_fn_builder(
            ...
            max_headers_size=FLAGS.max_headers_size, 
            max_extra_features_size=FLAGS.max_extra_features_size,
            is_training=True,
            drop_remainder=True)
    ...
    if FLAGS.do_eval:
        ...
        file_based_convert_examples_to_features(
            eval_examples, label_list, FLAGS.max_seq_length, tokenizer, eval_file,FLAGS.max_headers_size,FLAGS.max_extra_features_size)
    ...
    if FLAGS.do_predict:
        ...
        file_based_convert_examples_to_features(predict_examples, label_list,
                                                ...
                                                predict_file,FLAGS.max_headers_size,FLAGS.max_extra_features_size)
